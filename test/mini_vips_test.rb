...
class MiniVipsTest < Minitest::Test
  ...
  private_constant :FONT_PATHS, :PROFILE_PATHS
  ...
  def test_dominant_color_command
    Dir.mktmpdir do |directory|
      ...
      run_helper("topic-og", "--input", input_path, "--output", output_path)

      assert_equal "123456\n", run_helper("dominant-color", "--input", output_path)
    end
  end

  def test_topic_og_command
    Dir.mktmpdir do |directory|
      input_path = File.join(directory, "input.svg")
  ...
  def profile_path
    @profile_path ||=
      PROFILE_PATHS.find { |path| File.file?(path) } || raise("No test color profile found")
  end
end
